...
from . import util
from . import html
from PIL import Image #from scipy.misc import imresize
...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, f_name="", citysc=False):
    ...
    for label, im_data in visuals.items():
        ...
        if aspect_ratio < 1.0:
            im = np.array(Image.fromarray(obj=im).resize(size=(int(h / aspect_ratio), w), resample=Image.BICUBIC)) # im = imresize(im, (int(h / aspect_ratio), w), interp='bicubic')
